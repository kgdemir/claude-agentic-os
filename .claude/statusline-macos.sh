...
# Ported from Ubuntu to macOS (bash 3.2, BSD date/sed, no coreutils timeout).
...
to_epoch() {
  ...
  # BSD date can't parse fractional seconds (ccusage emits "...:00.000Z"); strip them.
  bsd_ts=$(printf '%s' "$ts" | sed -E 's/\.[0-9]+//; s/Z$/+0000/')
  date -u -j -f "%Y-%m-%dT%H:%M:%S%z" "$bsd_ts" +%s 2>/dev/null && return
...
# Run a command with a time limit; macOS has no coreutils timeout by default, perl alarm is the fallback.
run_with_timeout() {
  secs="$1"; shift
...
extract_json_string() {
  ...
  # ([0-9.][0-9.]* instead of \+ — BSD sed has no \+ in basic regex)
